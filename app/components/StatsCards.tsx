import { Flame, CheckCircle2, Ban, TimerOff, Lightbulb, Eye } from 'lucide-react';
  ...
  return (
    <div className="bg-surface-high p-8 rounded-[2rem] flex flex-col items-center text-center">
      ...
      <p className="text-5xl font-black mt-2 text-on-surface">
        5 <span className="text-lg font-medium text-on-surface-variant">days</span>
      </p>
...
export function TodayStatsCard() {
  const stats = [
    { label: 'Taken', value: 8, icon: CheckCircle2, color: 'text-secondary', bg: 'bg-secondary-container/10' },
    { label: 'Skipped', value: 2, icon: Ban, color: 'text-tertiary-container', bg: 'bg-tertiary-container/10' },
    { label: 'Missed', value: 3, icon: TimerOff, color: 'text-on-surface-variant', bg: 'bg-surface-highest' },
  ];
          ...
          <div key={stat.label}>
            <div className="flex items-center justify-between group">
              <div className="flex items-center gap-4">
                ...
                <span className="font-medium text-on-surface">{stat.label}</span>
              </div>
              <span className="text-xl font-bold font-manrope">{stat.value}</span>
            </div>
            {idx < stats.length - 1 && <div className="h-px bg-outline-variant/5 mt-6" />}
          </div>
        ))}
      </div>
    </div>
